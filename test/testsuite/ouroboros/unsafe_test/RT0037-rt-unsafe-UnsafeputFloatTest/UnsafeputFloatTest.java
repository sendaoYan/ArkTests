 ...
 * -@TestCaseID: UnsafeputFloatTest
 * -@TestCaseName: Unsafe api: putFloat()
 ...
 * -#step3:set value of step2 by Unsafe.putFloat
 * -#step4:check value of step3 correct
 * -@Expect:0\n
 * -@Priority: High
 * -@Source: UnsafeputFloatTest.java
 * -@ExecuteClass: UnsafeputFloatTest
 * -@ExecuteArgs:
 */
...
public class UnsafeputFloatTest {
    ...
    private static int run(String[] args, PrintStream out) {
        ...
        try {
            result = UnsafeputFloatTest_1();
        } catch (Exception e) {
            e.printStackTrace();
            UnsafeputFloatTest.res -= 2;
        }

        if (result == 3 && UnsafeputFloatTest.res == 97) {
            result =0;
        }
        return result;
    }

    private static int UnsafeputFloatTest_1() {
        Unsafe unsafe;
        Field field;
        Long offset;
        Field param;
        ...
        try {
            ...
            param = Billie32.class.getDeclaredField("aFloat");
            offset = unsafe.objectFieldOffset(param);
            obj = new Billie32();
            result = unsafe.getFloat(obj, offset);
            unsafe.putFloat(obj, offset, 11.11f);
            result = unsafe.getFloat(obj, offset);
            if (result == 11.11f) {
                UnsafeputFloatTest.res -= 2;
            }
