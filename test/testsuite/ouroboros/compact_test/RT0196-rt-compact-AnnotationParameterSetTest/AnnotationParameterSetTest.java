 ...
 *- @TestCaseName:AnnotationParameterSetTest.java
 *- @TestCaseName: AnnotationParameterSetTest
 *- @TestCaseType: Function Testing
 *- @RequirementID: AR000D0OR6
  *- @RequirementName: annotation
 *- @Title/Destination: positive test for User-defined Annotation with @Target(ElementType.PARAMETER)
 *- @Condition: no
 *- @Brief:no:
 * -#step1. User-defined Annotation with @Target(ElementType.PARAMETER) has field with primitive type, String, ENUM, annotation, class and their array
 * -#step2. verify able to set and get value
 *- @Expect:0\n
 *- @Priority: Level 1
 *- @Remark:
 *- @Source: AnnotationParameterSetTest.java AnnoA.java AnnoB.java
 *- @ExecuteClass: AnnotationParameterSetTest
 *- @ExecuteArgs:
 */

import java.util.Arrays;

public class AnnotationParameterSetTest {
    static int[] passCnt;
    public static void main(String[] args) throws NoSuchMethodException {
//        System.out.println(Test.class.getDeclaredMethod("a"));
        AnnoA anno = (AnnoA) Test.class.getDeclaredMethod("a", Object[].class).getParameterAnnotations()[0][0];
        passCnt = new int[24];
        // 基本数据类型
        int i = 0;
...
class Test{
    private Thread.State[] a(@AnnoA(intA = Integer.MAX_VALUE, byteA = Byte.MAX_VALUE, charA = Character.MAX_VALUE, doubleA = Double.NaN,
            ...
            stateAA = Thread.State.NEW, annoBA = @AnnoB, annoBAA = {@AnnoB, @AnnoB}) Object... aaa){
