 ...
 *- @TestCaseName:AnnotationMethodSetTest.java
 *- @TestCaseName: AnnotationMethodSetTest
 *- @TestCaseType: Function Testing
 *- @RequirementID: AR000D0OR6
  *- @RequirementName: annotation
 *- @Title/Destination: positive test for User-defined Annotation with @Target(ElementType.METHOD)
 *- @Condition: no
 *- @Brief:no:
 * -#step1. User-defined Annotation with @Target(ElementType.METHOD) has field with primitive type, String, ENUM, annotation, class and their array
 * -#step2. verify able to set and get value
 *- @Expect:0\n
 *- @Priority: Level 1
 *- @Remark:
 *- @Source: AnnotationMethodSetTest.java AnnoA.java AnnoB.java
 *- @ExecuteClass: AnnotationMethodSetTest
 *- @ExecuteArgs:
 */

import java.util.Arrays;

public class AnnotationMethodSetTest {
    static int[] passCnt;
    public static void main(String[] args) throws NoSuchMethodException {
        AnnoA anno = Test.class.getDeclaredMethod("a").getAnnotation(AnnoA.class);
        passCnt = new int[24];
        // primitive type
        int i = 0;
