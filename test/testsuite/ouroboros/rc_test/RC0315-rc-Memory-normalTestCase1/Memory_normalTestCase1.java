/* @test */
/*
 *- @TestCaseID:rc/stress/Memory_normalTestCase1.java
 *- @TestCaseName:MyselfClassName
 *- @RequirementName:[运行时需求]支持自动内存管理
 *- @Title/Destination:默认启动80个线程，对62个runslot去申请释放它们所管理的区间大小的内存块,所申请的内存块不会立刻释放。
 *- @Condition: no
 * -#c1
 *- @Brief:functionTest
 * -#step1
 *- @Expect:
 *- @Priority: High
 *- @Source: Memory_normalTestCase1.java
 *- @ExecuteClass: Memory_normalTestCase1
 *- @ExecuteArgs:
 *- @Remark:
...
public class Memory_normalTestCase1 {
    ...
    public static void testCase1() {
        ...
        for (int i = 0; i < DEFAULT_THREAD_NUM; i++) {
            slot = i % MAX_SLOT_NUM;
            t = new Thread(new MemAlloc(ALLOC_4K, slot, mAllThreadReference, i, MemAlloc.SAVE_ALL, false, 1), "testCase1_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
