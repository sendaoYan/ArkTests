/* @test */
/*
 *- @TestCaseID:rc/stress/Memory_normalTestCase2.java
 *- @TestCaseName:MyselfClassName
 *- @RequirementName:[运行时需求]支持自动内存管理
 *- @Title/Destination:默认启动80个线程，对62个runslot去申请释放它们所管理的区间大小的内存块，一起释放。同时在起一个线程去申请同样大小的内存块去一半一半地释放。这样会造成大量的释放
 *- @Condition: no
 * -#c1
 *- @Brief:functionTest
 * -#step1
 *- @Expect:
 *- @Priority: High
 *- @Source: Memory_normalTestCase2.java
 *- @ExecuteClass: Memory_normalTestCase2
 *- @ExecuteArgs:
 *- @Remark:
...
public class Memory_normalTestCase2 {
    ...
    public static void testCase2() {
        ...
        for (int i = 0; i < DEFAULT_THREAD_NUM; i++) {
            slot = i % MAX_SLOT_NUM;
            Runnable remain = new MemAlloc(ALLOC_4K, slot, mAllThreadReference, i, MemAlloc.SAVE_HALF_RAMDOM, true, 1);
            t = new Thread(new MemAlloc(ALLOC_4K, slot, mAllThreadReference, i, MemAlloc.SAVE_ALL, true, 1, DEFAULT_REPEAT_TIMES, remain), "testCase2_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
