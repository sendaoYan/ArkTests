 ...
 * -#step5：创建InCycle的实例，调用方法setCycle给第一个类对象关于第二个类的Field初始化对象，判断对象的引用不为空，再将该对象关于
 *          第一个类Field赋值为第一个类，从而形成一个环。
 * -#step6：方法setCycle调用方法ModifyA1执行前两个类对象的add方法进行Field的运算，相当于对对象进行了引用，判断运算结果是否正确。
 * -#step7：等待一个回收周期，确认两个类的软引用、弱引用、虚引用对象为NULL。
 * -#step8：调用Runtime.getRuntime().gc()进行资源回收，重复步骤4~7。
 * -#step9：确认class A1的软引用、弱引用、虚引用都应该为空。
 *- @Expect: ExpectResult\n
 *- @Priority: High
 *- @Source:NotIsCleanerInDeadCycleNotFreeRefFailSetWCBFailAtomicWCB.java
 *- @ExecuteClass: NotIsCleanerInDeadCycleNotFreeRefFailSetWCBFailAtomicWCB
 *- @ExecuteArgs:
 *- @Remark:
 */
