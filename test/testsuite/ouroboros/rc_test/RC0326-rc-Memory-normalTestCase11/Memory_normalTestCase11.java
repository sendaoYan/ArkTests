/* @test */
/*
 * -@TestCaseID:rc/stress/Memory_normalTestCase11.java
 ...
 * -@Expect:
 * -@Priority: High
 * -@Source: Memory_normalTestCase11.java
 * -@ExecuteClass: Memory_normalTestCase11
 * -@ExecuteArgs:
 * -@Remark:
...
public class Memory_normalTestCase11 {
    private static final int DEFAULT_THREAD_NUM = 80;
    private static final int MAX_SLOT_NUM = 62;
    private static boolean rest = false;
    private static int restTime = 5000;
    private static boolean running = true;
    private static final int ALLOC_4K = 4 * 1024;
    private static final int MAX_REALIVE_OBJ_NUM = 1000;
    private static List<List<AllocUnit>> allThreadReference = new ArrayList<List<AllocUnit>>();
    ...
    private static void trySleep(long time) {
        try {
            ...
            throw new RuntimeException("Thread interrupted during trySleep", e);
        }
    }
    ...
    private static void startAllThread(List<Thread> list) {
        for (Thread s : list) {
            s.start();
    ...
    private static void waitAllThreadFinish(List<Thread> list) {
        for (Thread s : list) {
            try {
                s.join();
            } catch (InterruptedException e) {
                throw new RuntimeException("Thread interrupted while joining", e);
            }
        }
    }

    private static void testCase11() {
        resetTestEnvirment(allThreadReference);
        ArrayList<Thread> list = new ArrayList<Thread>();
        Thread t = null;
        for (int i = 0; i < DEFAULT_THREAD_NUM; i++) {
            int slot = i % MAX_SLOT_NUM;
            t = new Thread(new MemAlloc(ALLOC_4K, slot, allThreadReference, i, MemAlloc.SAVE_ALL,
                false, 1), "testCase11_alloc_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
        ...
        list.clear();
        for (int i = 0; i < DEFAULT_THREAD_NUM; i++) {
            t = new Thread(new FreeReference(allThreadReference, i, false), "testCase11_free_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
        waitAllThreadFinish(list);
    ...
    public abstract static class MemAllocMethod {
    ...
    static class MemAlloc implements Runnable {
        ...
        public static final int PAGE_SIZE = 4016;
        public static final int PAGE_HEADSIZE = 80;
        public static final int OBJ_MAX_SIZE = 1008;
        public static final int OBJ_HEADSIZE = 16;
        ...
        public MemAlloc(boolean isStress, int allocSize, int slot, List<List<AllocUnit>> saveTo, int saveIndex,
            int saveNum, boolean freeImmediately, int sleepTime, int repeatTimes,
            Runnable callBackAfterFree, MemAllocMethod allMethod) {
            mIsStress = isStress;
        ...
        public MemAlloc(int allocSize, int slot, List<List<AllocUnit>> saveTo, int saveIndex, int saveNum,
            boolean freeImmediately, int sleepTime, int repeatTimes,
            Runnable callBackAfterFree, MemAllocMethod allMethod) {
            this(false, allocSize, slot, saveTo, saveIndex, saveNum, freeImmediately, sleepTime, repeatTimes,
                callBackAfterFree, null);
        }

        public MemAlloc(int allocSize, int slot, List<List<AllocUnit>> saveTo, int saveIndex, int saveNum,
            boolean freeImmediately, int sleepTime, int repeatTimes, Runnable callBackAfterFree) {
            this(allocSize, slot, saveTo, saveIndex, saveNum, freeImmediately, sleepTime, repeatTimes,
                callBackAfterFree, null);
        }
         ...
         **/
        public MemAlloc(int allocSize, int slot, List<List<AllocUnit>> saveTo, int saveIndex, int saveNum,
            boolean freeImmediately, int sleepTime) {
            this(allocSize, slot, saveTo, saveIndex, saveNum, freeImmediately, sleepTime, 1);
        }

        public MemAlloc(boolean isStress, int allocSize, int slot, List<List<AllocUnit>> saveTo, int saveIndex,
            int saveNum, boolean freeImmediately, int sleepTime, int repeatTimes,Runnable callBackAfterFree) {
            this(isStress, allocSize, slot, saveTo, saveIndex, saveNum, freeImmediately, sleepTime, repeatTimes,
                callBackAfterFree, null);
        }

        public MemAlloc(boolean isStress, int allocSize, int slot, List<List<AllocUnit>> saveTo, int saveIndex,
            int saveNum, boolean freeImmediately, int sleepTime, int repeatTimes) {
            this(isStress, allocSize, slot, saveTo, saveIndex, saveNum, freeImmediately, sleepTime,
                repeatTimes, null);
        }

        public MemAlloc(boolean isStress, int allocSize, int slot, List<List<AllocUnit>> saveTo, int saveIndex,
            int saveNum, boolean freeImmediately, int sleepTime) {
        ...
        public void run() {
            for (int j = 0; j < mRepeatTimes || mRepeatTimes < 0; j++) {
                ...
                if (mSaveTo != null && mSaveNum != 0) {
                    if (mSaveNum == SAVE_HALF && mSaveIndex < mSaveTo.size() && ret != null) {
                        int half = ret.size() / 2;
