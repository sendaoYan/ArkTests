/* @test */
/*
 *- @TestCaseID:rc/stress/Memory_normalTestCase12.java
 ...
 *- @Expect:
 *- @Priority: High
 *- @Source: Memory_normalTestCase12.java
 *- @ExecuteClass: Memory_normalTestCase12
 *- @ExecuteArgs:
 *- @Remark:
...
public class Memory_normalTestCase12 {
    ...
    public static void testCase11() {
        ...
        for (int i = 0; i < DEFAULT_THREAD_NUM; i++) {
            ...
            t = new Thread(new MemAlloc(ALLOC_4K, slot, mAllThreadReference, i, MemAlloc.SAVE_ALL, false, 1), "testCase11_alloc_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
        ...
        list.clear();
        for (int i = 0; i < DEFAULT_THREAD_NUM; i++) {
            t = new Thread(new FreeReference(mAllThreadReference, i, false), "testCase11_free_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
        waitAllThreadFinish(list);
