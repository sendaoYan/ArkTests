/* @test */
/*
 *- @TestCaseID:rc/stress/Memory_normalTestCase5.java
 ...
 *- @Expect:
 *- @Priority: High
 *- @Source: Memory_normalTestCase5.java
 *- @ExecuteClass: Memory_normalTestCase5
 *- @ExecuteArgs:
 *- @Remark:
...
public class Memory_normalTestCase5 {
    ...
    public static void testCase5() {
        ...
        Thread t = null;
        int average = ALLOC_4K / DEFAULT_THREAD_NUM;
        for (int i = 0; i < DEFAULT_THREAD_NUM; i++) {
            int slot = i % MAX_SLOT_NUM;
            t = new Thread(new MemAlloc(ALLOC_4K + Math.max(getSlotSize(slot), average), slot, mAllThreadReference, i, MemAlloc.SAVE_ALL, false, 1), "testCase5_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
