 ...
 *- @Expect:
 *- @Priority: High
 *- @Source: Memory_normalTestCase4.java
 *- @ExecuteClass: Memory_normalTestCase4
 *- @ExecuteArgs:
 *- @Remark:
...
public class Memory_normalTestCase4 {
    ...
    public static void testCase4() {
        ...
        for (int i = 0; i < MAX_SLOT_NUM; i++) {
            slot = i % MAX_SLOT_NUM;
            Runnable remain = new MemAlloc(ALLOC_2K, slot, mAllThreadReference, i, MemAlloc.SAVE_HALF_RAMDOM, true, 1);
            t = new Thread(new MemAlloc(ALLOC_4K, slot, mAllThreadReference, i, MemAlloc.SAVE_HALF_RAMDOM, false, 1, DEFAULT_REPEAT_TIMES, remain), "testCase4_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
