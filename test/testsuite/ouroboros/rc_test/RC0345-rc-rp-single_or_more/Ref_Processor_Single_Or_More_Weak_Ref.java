/* @test */
/*
 *- @TestCaseID: maple/runtime/rc/function/Ref_Processor_Single_Or_More_Weak_Ref.java
 *- @TestCaseName:MyselfClassName
 *- @RequirementName:[运行时需求]支持自动内存管理
 *- @Title/Destination: Reference Processor: single weak/soft phantom reference.
 *- @Condition: no
 *- @Brief:functionTest
 * -#step1: 创建弱引用，并判断创建后引用不为空, 等待一个时间，引用应该被回收为空，引用队列也为空。
 * -#step2: 创建软引用，并判断创建后引用不为空, 等待一个时间，引用应该被回收为空，引用队列也为空。
 * -#step3: 创建虚引用，并判断创建后引用不为空, 等待一个时间，引用应该被回收为空，引用队列也为空。
 * -#step4: 同时创建弱引用、软引用、虚引用，并判断创建后引用不为空, 等待一个时间，引用应该被回收为空，引用队列也为空。
 * -#step5: 调用Runtime.getRuntime().gc()进行资源回收。
 * -#step6: 重复步骤1~4。
 *- @Expect: ExpectResult\n
 *- @Priority: High
 *- @Source: Ref_Processor_Single_Or_More_Weak_Ref.java
 *- @ExecuteClass: Ref_Processor_Single_Or_More_Weak_Ref
 *- @ExecuteArgs:
 *- @Remark:
 */
