/* @test */
/*
 * -@TestCaseID:rc/stress/Memory_normalTestCase9.java
 ...
 * -@Expect:
 * -@Priority: High
 * -@Source: Memory_normalTestCase9.java
 * -@ExecuteClass: Memory_normalTestCase9
 * -@ExecuteArgs:
 * -@Remark:
...
public class Memory_normalTestCase9 {
    ...
    static List<List<AllocUnit>> allThreadReference = new ArrayList<List<AllocUnit>>();
    ...
    private static void testCase9() {
        ...
        for (int i = 0; i < DEFAULT_THREAD_NUM; i++) {
            ...
            t = new Thread(new MemAlloc(ALLOC_12K, slot, allThreadReference, i, MemAlloc.SAVE_ALL,
                false, 1), "testCase9_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
    ...
    static class MemAlloc implements Runnable {
        ...
        public MemAlloc(boolean isStress, int allocSize, int slot, List<List<AllocUnit>> saveTo, int saveIndex,
            ...
            this(isStress, allocSize, slot, saveTo, saveIndex, saveNum, freeImmediately, sleepTime,
                repeatTimes, callBackAfterFree, null);
        }
