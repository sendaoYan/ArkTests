/* @test */
/*
 *- @TestCaseID:Alloc_Thread_192x8B
 *- @TestCaseName:MyselfClassName
 *- @RequirementName:[运行时需求]支持自动内存管理
 *- @Title:ROS Allocator is in charge of applying and releasing objects.This mulit thread testcase is mainly for testing objects from 128*8B to 192*8B(max)
 *- @Condition: no
 * -#c1
 *- @Brief:functionTest
 * -#step1
 *- @Expect:ExpectResult\n
 *- @Priority: High
 *- @Source: Alloc_Thread_192x8B.java
 *- @ExecuteClass: Alloc_Thread_192x8B
 *- @ExecuteArgs:
 *- @Remark:
 */
import java.util.ArrayList;

class Alloc_Thread_192x8B_01 extends Thread {
...
public class Alloc_Thread_192x8B {
    public static void main(String[] args){
        Alloc_Thread_192x8B_01 test1=new Alloc_Thread_192x8B_01();
        test1.start();
        try{
            test1.join();
