 ...
 *- @Expect:
 *- @Priority: High
 *- @Source: Memory_normalTestCase6_1.java
 *- @ExecuteClass: Memory_normalTestCase6_1
 *- @ExecuteArgs:
 *- @Remark:
...
public class Memory_normalTestCase6_1 {
    ...
    public static void testCase6_1() {
        ...
        Thread t = null;
        t = new Thread(new MemAlloc(ALLOC_4K, (int) (Math.random() * MAX_SLOT_NUM), mAllThreadReference, 0, 1, false, 1, DEFAULT_THREAD_NUM), "testCase6_1_" + (1));
        list.add(t);
        startAllThread(list);
        ...
        for (int i = 1; i < DEFAULT_THREAD_NUM + 1; i++) {
            ...
            t = new Thread(new MemAlloc(ALLOC_8K, slot, mAllThreadReference, i, MemAlloc.SAVE_ALL, false, 1), "testCase6_1_" + (i + 1));
            list.add(t);
        }
        startAllThread(list);
        waitAllThreadFinish(list);
