 ...
 *- @Condition: no
 *- @Brief:functionTest
 * -#step1: 分别创建Cycle_A、Cycle_B、Cycle_C、Cycle_D 四个类的对象，均含有弱引用，并且这四个对象互为循环引用；
 ...
 *- @Expect: ExpectResult\n
 *- @Priority: High
 *- @Source: Ref_Processor_Cycle_All_Weak_Ref.java
 *- @ExecuteClass: Ref_Processor_Cycle_All_Weak_Ref
 *- @ExecuteArgs:
 *- @Remark:
 */
...
public class Ref_Processor_Cycle_All_Weak_Ref {
...
class Cycle_D {
    Reference str;
    Cycle_A cya;
    StringBuffer stringBufferD = new StringBuffer("ref_processor_cycle_D");
    int dNum;
    int sum;

    Cycle_D() {
        stringBufferD = new StringBuffer("ref_processor_cycle_D");
